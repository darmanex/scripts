#!/bin/sh
#
# This script is for pruning builds on OKD (Openshift) master node
# To make this script automated execution, please put on cronjob
# eg: execute at 00:00 on day-of-month 1
# 0 0 1 * * /path/of_your_scripts/builds-prune.sh
#
...
LOGDIR=/root/logs
CURDATE=$(date)
LOGFILE=$LOGDIR/builds-prune.log

oc login $OKD_URL:$OKD_PORT -u=$OKD_UNAME -p=$OKD_PASS --insecure-skip-tls-verify

oc adm prune builds --orphans --keep-complete=3 --keep-failed=1 --keep-younger-than=43200m --confirm > $LOGFILE
